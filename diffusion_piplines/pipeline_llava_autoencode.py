...
class EmuVisualGenerationPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.gen_vision_tower = self.multimodal_encoder.model.get_gen_vision_tower()
        device = torch.device("cuda")
        self.gen_vision_tower.load_model()
        self.gen_vision_tower.to(device)
        self.gen_image_processor = self.gen_vision_tower.image_processor
    ...
    def __call__(
        self,
        inputs: List[Image.Image] | Image.Image,
        height: int = 1024,
        width: int = 1024,
        ...
        prompt_embeds = self._prepare_and_encode_inputs(inputs).to(dtype).to(device)
    ...
    def _prepare_and_encode_inputs(
        ...
        for x in inputs:
            ...
            image_inputs = self.gen_image_processor.preprocess(resized_images, return_tensors="pt")
        ...
        prompt_image_embeds = self.gen_vision_tower(image_prompt.cuda()) # TODO: check dimension
      
        if 'early' in self.multimodal_encoder.get_gen_pooling():
            prompt_image_embeds = self.multimodal_encoder.pool_img(prompt_image_embeds)
        prompt = prompt_image_embeds
        prompt = prompt.view(1, 1792, -1).permute(0,2,1).contiguous()
        return prompt
